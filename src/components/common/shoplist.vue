<template>
    <div class="shoplist_container">
        <ul v-load-more="loaderMore" v-if="shopListArr.length" type="1">
			<router-link class="shop_li" tag="li" v-for="item in shopListArr" :key="item.id" :to="{path:'shop', query:{geohash, id:item.id}}">
				<section>
		...
		<div ref="abc" style="background-color: #f00;"></div>
...
export default {
	...
	data() {
		return {
			offset: 0,  //批次加载商铺列表，每次加载20个（limit=20）
			shopListArr: [],
	...
	methods: {
		...
		//到达底部加载更多数据
		async loaderMore() {
			console.log('shoplist.vue--loaderMore--this.touchend='+this.touchend+', this.preventRepeatReuqest='+this.preventRepeatReuqest);
