...
export default {
    ...
    [types.REDUCE_CART](state, { shopid, category_id, item_id, food_id, name, price, specs }) {
        ...
        if (item && item[food_id]) {
            if (item[food_id]['num'] > 0) {
                ...
                item[food_id] = null;  //商品数量为0，则清空当前商品的信息
            }
        }
    },
    ...
    [types.CONFIRM_REMARK](state, { remarkText, inputText }) {
    ...
    //保存下单后购物id和sig
    [types.SAVE_CART_ID_SIG](state, { cartId, sig }) {
        state.cartId = cartId;
        state.sig = sig;
    },
    ...
    //删除地址列表
    [types.SAVE_ADDRESS](state, newAddress) {
