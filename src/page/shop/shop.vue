        ...
        <section class="shop_container" v-if="!showLoading">
            <nav class="goback" @click="goBack">
                <svg width="4rem" height="100%" xmlns="http://www.w3.org/2000/svg" version="1.1">
                    <polyline points="12,18 4,9 12,0" style="fill:none; stroke:#fff; stroke-width:3;"></polyline>
                </svg>
            </nav>
            <header class="shop_detail_header" ref="shopHeader" :style="{zIndex: showActivities?'14':'10'}">
                ...
                <section class="description_header">
                    <router-link class="description_top" to="/shop/shopDetail">
                        ...
                        <svg width="14" height="14" xmlns="http://www.w3.org/2000/svg" version="1.1" class="description_arrow">
                            <path d="M0 0 L8 7 L0 14" style="fill:none; stroke:#fff; stroke-width:1;"></path>
                        </svg>
                    </router-link>
                    <footer class="description_footer" v-if="shopDetailData.activities.length" @click="showActivitiesFun">
                        <p class="ellipsis">
                            <span class="tip_icon" :style="{backgroundColor:'#'+shopDetailData.activities[0].icon_color, borderColor:'#'+shopDetailData.activities[0].icon_color}">{{shopDetailData.activities[0].name}}</span>
            ...
            <transition name="fade">
                <section class="activities_details" v-if="showActivities">
                    ...
                    <svg width="60" height="60" class="close_activities" @click.stop="showActivitiesFun">
                        <circle cx="30" cy="30" r="25" style="fill:none; stroke:#555; stroke-width:1;"></circle>
            ...
            <section class="change_show_type" ref="chooseType">
                <div><span :class="{activity_show: changeShowType=='food'}" @click="changeShowType='food'">商品</span></div>
                <div><span :class="{activity_show: changeShowType=='rating'}" @click="changeShowType='rating'">评价</span></div>
            </section>
                            ...
                            <ul>
                                <li v-for="(item, index) in menuList" :key="index">
                                    ...
                                    <section class="menu_detail_list" v-for="(foods, foodIndex) in item.foods" :key="foodIndex">
                                        <router-link tag="div" class="menu_detail_link" :to="{path: 'shop/foodDetail', query:{image_path: foods.image_path, description:foods.description, month_sales:foods.month_sales, name:foods.name, rating:foods.rating, rating_count:foods.rating_count, satisfy_rate:foods.satisfy_rate, foods, shopId}}">
                    ...
                    <section v-load-more="loaderMoreRating" type="2">
                        <section>
                            ...
                            <ul class="tag_list_ul">
                                <li v-for="(item, index) in ratingTagsList" :key="index" :class="{unsatisfied: item.unsatisfied, tagActivity: ratingTagsIndex==index}" @click="changeTagIndex(index, item.name)">{{item.name}}({{item.count}})</li>
                            </ul>
                ...
                <div class="specs_list" v-if="showSpecs">
                    <header class="specs_list_header">
                        ...
                        <svg class="specs_cancel" width="16" height="16" xmlns="http://www.w3.org/2000/svg" version="1.1" @click="showChooseList">
                            <line x1="0" y1="0" x2="16" y2="16" style="fill:none; stroke:#666; stroke-width:1.2;"></line>
                            <line x1="0" y1="16" x2="16" y2="0" style="fill:none; stroke:#666; stroke-width:1.2;"></line>
                        </svg>
                    </header>
                    <section class="specs_details">
                        <h5 class="specs_details_title">{{choosedFoods.specifications[0].name}}</h5>
                        <ul>
                            <li v-for="(item, itemIndex) in choosedFoods.specifications[0].values" :key="itemIndex" :class="{specs_activity: itemIndex==specsIndex}" @click="chooseSpecs(itemIndex)">{{item}}</li>
                        </ul>
                    </section>
...
export default {
    ...
    computed: {
        ...
        promotionInfo(){
            return this.shopDetailData.promotionInfo||'欢迎光临，用餐高峰期请提前下单，谢谢。';
        },
        //配送费
        deliveryFee() {
    ...
    methods: {
        ...
        //滑动食品列表时，监听其scrollTop值来设置对应的食品列表标题的样式
        ...
        //初始化和shopCart变化时，重新获取购物车改变过的数据，赋值
        initCategoryNm() {
            let newArr = [];
            let cartFoodNum = 0;
            this.totalPrice = 0;
    ...
    watch: {
        ...
        changeShowType: function(value) {
            console.log('shop.vue--changeShowType--value='+value);
            if(value==='rating') {
                this.$nextTick(()=>{
